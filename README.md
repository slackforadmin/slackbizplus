...
function nextStep(currentId, nextId, type, progress) {
    // 현재 단계 숨기기
    document.getElementById('step-' + currentId).style.display = 'none';
    // 다음 단계 보여주기
    const nextEl = document.getElementById('step-' + nextId + (type && nextId !== 3 ? '-' + type : ''));
